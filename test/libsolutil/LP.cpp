...
public:
	...
	{
		return factorForVariable(variableIndex(_name), 1);
	}
...
BOOST_AUTO_TEST_CASE(bounds2)
{
	...
	addEQConstraint(add(variable("y"), constant(231)), variable("x"));
	feasible({{"x", 234}, {"y", 3}});

	addEQConstraint(add(variable("y"), constant(10)), variable("x") - variable("z"));
	feasible({{"x", 234}, {"y", 3}});

	addEQConstraint(add(variable("z"), variable("x")), constant(2));
	infeasible();
}
...
BOOST_AUTO_TEST_CASE(lower_bound)
{
	...
	addLEConstraint(add(2 * variable("x"), variable("y")), 2);
	feasible({{"x", 0}, {"y", 2}});
}
...
BOOST_AUTO_TEST_CASE(unbounded2)
{
	auto x = variable("x");
	auto y = variable("y");
	addLEConstraint(constant(2), add(x, y));
...
BOOST_AUTO_TEST_CASE(unbounded3)
{
	...
	addEQConstraint(add(variable("y"), constant(3)), variable("x"));
	feasible({{"x", 3}, {"y", 0}});

	addLEConstraint(add(variable("y"), variable("x")), constant(2));
	infeasible();
}
...
BOOST_AUTO_TEST_CASE(equal)
{
	auto x = variable("x");
	auto y = variable("y");
	addEQConstraint(x, add(y, constant(10)));
...
BOOST_AUTO_TEST_CASE(linear_dependent)
{
	...
	addLEConstraint(add(add(x, y), z), 100);
	feasible({{"x", 5}, {"y", 5}, {"z", 5}});

	addLEConstraint(add(add(x, y), z), 2);
	feasible({{"x", 2}, {"y", 0}, {"z", 0}});

	addLEConstraint(constant(2), add(add(x, y), z));
	feasible({{"x", 2}, {"y", 0}, {"z", 0}});

	addEQConstraint(constant(2), add(add(x, y), z));
	feasible({{"x", 2}, {"y", 0}, {"z", 0}});
}
